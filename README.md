...
| 学校/专业 + 论文选题需求 | 从上市公司案例生成可写、可答辩、与职业方向相关的论文题目 |
...
提供学校、专业、培养类型与未来就业方向（可选），以及指定的公司或没有公司。GXG 会从上市公司等可核验案例中发现真实商业问题，生成可写、可答辩、与职业方向相关的论文题目，并输出便于学生直接作决定的《论文选题建议》。严格不提供论文代写。

## 直接调用的 Skill

GXG 当前正式包含 1 个主入口和 4 个子 Skill：
...
| 选一个能写、能答辩的论文题目 | `@gxg-thesis-topic` | 候选题与《论文选题建议》、可写性与职业方向说明 |
...
安装完成后，新建一个对话并输入 `@gxg`。如果当前平台不支持从主 Skill 动态加载子 Skill，可直接输入 `@gxg-job-fit`、`@gxg-interview`、`@gxg-insight` 或 `@gxg-thesis-topic`。
